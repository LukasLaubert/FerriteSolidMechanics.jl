...
Implementing a finite-strain constitutive model directly in Ferrite.jl requires cell iteration, sub-dof-handler setup, quadrature handling, optional thread/MPI parallelism, and state management for each model. This package factors that out by providing:
...
- **Bundled material models** ranging from linear elasticity to finite-strain hyperelastic and viscoelastic-viscoplastic laws (`MooneyRivlin`, `Ogden`, `ArrudaBoyce`, `VEPD_Detrez2010`, `VEVP_Zhao2021_AD`, `VEVP_MOAMMM`).

To add a new material, implement the hooks for that material. The [Developer guide](developer_guide.md) walks through the smallest possible example using the bundled `J2Plasticity` model.
